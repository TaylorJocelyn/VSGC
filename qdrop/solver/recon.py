...
from qdrop.solver.imagenet_utils import DataSaverHook, StopForwardException
...
def save_inp_oup_data(model, module, cali_img, cali_t, cali_y, store_inp=False, store_oup=False, bs: int = 32, keep_gpu: bool = True):
    ...
    cached = [[], []]
    with torch.no_grad():
        for i in range(int(cali_img.size(0) / bs)):
            try:
                _ = model(cali_img[i * bs: (i + 1) * bs].to(device), cali_t[i * bs: (i + 1) * bs].to(device), cali_y[i * bs: (i + 1) * bs].to(device))
...
def save_block_inp_oup_data(model, module, cali_img, cali_t, cali_y, store_inp=False, store_oup=False, bs: int = 32, keep_gpu: bool = True):
    ...
    with torch.no_grad():
        for i in range(int(cali_img.size(0) / bs)):
            ...
            if store_inp:
                if keep_gpu:
                    cached[0].append(data_saver.input_store[0].detach())
                    cached[1].append(data_saver.input_store[1].detach())
                else:
                    cached[0].append(data_saver.input_store[0].detach().cpu())
                    cached[1].append(data_saver.input_store[1].detach().cpu())
            if store_oup:
                if keep_gpu:
                    cached[2].append(data_saver.output_store.detach())
                else:
                    cached[2].append(data_saver.output_store.detach().cpu())
    if store_inp:
        cached[0] = torch.cat([x for x in cached[0]])
        cached[1] = torch.cat([x for x in cached[1]])
    if store_oup:
        cached[2] = torch.cat([x for x in cached[2]])
    handle.remove()
    torch.cuda.empty_cache()
    return cached
...
class LossFunction:
    ...
    def __init__(self,
                 module: Union[QuantizedModule, ResBlock, BasicTransformerBlock],
                 weight: float = 1.,
                 iters: int = 20000,
                 b_range: tuple = (20, 2),
                 warm_up: float = 0.0,
...
def layer_reconstruction(model, fp_model, module, fp_module, cali_img, cali_t, cali_y, config):
    device = next(module.parameters()).device
    # get data first
    quant_inp, _ = save_inp_oup_data(model, module, cali_img, cali_t, cali_y, store_inp=True, store_oup=False, bs=config.batch_size, keep_gpu=config.keep_gpu)
    fp_inp, fp_oup = save_inp_oup_data(fp_model, fp_module, cali_img, cali_t, cali_y, store_inp=True, store_oup=True, bs=config.batch_size, keep_gpu=config.keep_gpu)
...
def block_reconstruction(model, fp_model, module, fp_module, cali_img, cali_t, cali_y, config):
    device = next(module.parameters()).device
    # get data first
    quant_inp0, quant_inp1, _ = save_block_inp_oup_data(model, module, cali_img, cali_t, cali_y, store_inp=True, store_oup=False, bs=config.batch_size, keep_gpu=config.keep_gpu)
    fp_inp0, fp_inp1, fp_oup = save_block_inp_oup_data(fp_model, fp_module, cali_img, cali_t, cali_y, store_inp=True, store_oup=True, bs=config.batch_size, keep_gpu=config.keep_gpu)
    # prepare for up or down tuning
    w_para, a_para = [], []
    for name, layer in module.named_modules():
        if isinstance(layer, (QLinear, QConv2d)):
    ...
    sz = quant_inp0.size(0)
    for i in range(config.iters):
        idx = torch.randint(0, sz, (config.batch_size,))
        if config.drop_prob < 1.0:
            cur_quant_inp0 = quant_inp0[idx].to(device)
            cur_quant_inp1 = quant_inp1[idx].to(device)

            cur_fp_inp0 = fp_inp0[idx].to(device)
            cur_fp_inp1 = fp_inp1[idx].to(device)

            cur_inp0 = torch.where(torch.rand_like(cur_quant_inp0) < config.drop_prob, cur_quant_inp0, cur_fp_inp0)
            cur_inp1 = torch.where(torch.rand_like(cur_quant_inp1) < config.drop_prob, cur_quant_inp1, cur_fp_inp1)
        else:
            cur_inp0 = quant_inp0[idx].to(device)
            cur_inp1 = quant_inp1[idx].to(device)
        cur_fp_oup = fp_oup[idx].to(device)
        if a_opt:
            a_opt.zero_grad()
        w_opt.zero_grad()
        cur_quant_oup = module(cur_inp0, cur_inp1)
        err = loss_func(cur_quant_oup, cur_fp_oup)
        err.backward()
        w_opt.step()
        if a_opt:
